...
## Workspace State
- **Existing Code**: No
- **Programming Languages**: N/A (sin código de app aún)
- **Build System**: N/A
- **Project Structure**: Empty (greenfield — solo docs, PRD, specs, framework AI-DLC)
- **Reverse Engineering Needed**: No
- **Workspace Root**: /Users/mauricio/dev/perito

## Domain Inputs
- `PRD.md` — Product Requirements Document (Estación 2), input principal de AI-DLC.
- `AGENTS.md` — contexto de negocio/arquitectura y principios no negociables P1-P7.
- `.claude/rules/` — reglas duras: hitl (P1), coverage-determinism (P2), termination (P4), testing.
...
- **Stages to Execute (Inception)**: Application Design, Units Generation.
- **Stages to Execute (Construction, sesión posterior)**: Functional Design, NFR Requirements, NFR Design, Code Generation, Build & Test.
- **Stages to Skip**: Reverse Engineering (greenfield), Infrastructure Design (portafolio — nada se despliega, RES-02/P7; Won't).
- **Alcance activo de esta sesión**: hasta Units Generation + Arquitectura Just-in-Time → luego cosecha a `specs/aidlc/`.
